  ...
  const handleReleaseFunds = async () => {
    ...
    try {
      const res = await axiosInstance.post(
        `investment/bids/${acceptedId}/release-funds/`
      );
        ...
        <div className="px-6 pb-6">
          <div className="mb-10 pt-6">
            <p className="text-sm font-poppinsRegular text-[#5C6C71]">
              You`re about to release funds from your wallet for this project.
              Farmpady will deduct the bid amount and send it to the farmer once
          ...
          <div className="flex gap-x-4">
            ...
            <Button onClick={handleReleaseFunds} className="w-full">
              {loading ? "Releasing..." : "Yes, Release Funds"}
            </Button>
          </div>
        </div>
      </div>
